...
bool ratInMaze(char maze[10][10], int solution[][10], int i, int j, int m, int n) {
    ...
    bool RightSide = ratInMaze(maze, solution, i, j + 1, m, n); // i resemble the right side 
    bool DownSide = ratInMaze(maze, solution, i + 1, j, m, n); // j resemble the down side 

    //backtracking (fill the visited case with 0 while comes backward to the same path)
    solution[i][j] = 0;

    //wether rightside or downside is possible consider it as true for visiting
